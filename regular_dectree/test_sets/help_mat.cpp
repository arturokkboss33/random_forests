...
//Variables for parsing the data file
	std::string filename = "SPECT.test";
	std::string line;
	...
	cv::FileStorage fstore_traindata("spect_test.yml",cv::FileStorage::WRITE);
	cv::Mat train_samples(train_set.colRange(1,train_set.cols));
	fstore_traindata << "test_samples" << train_samples;
	fstore_traindata << "test_labels" << labels;
